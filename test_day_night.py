...
class UI:
    ...
    def draw_base_sun_level_buttons(self) -> None:
        font = pygame.font.SysFont('Arial', 16)
        self.draw_button(screen, pygame.Rect(300, 10, 40, 40), "-", 18, 10)
        sun_text = font.render(f'Sun: {self.simulation.base_sun_level}', True, self.icon_color)
    ...
    def draw_time_of_day(self) -> None:
        time_names = ["Morning", "Day", "Evening", "Night"]
        time_colors = [(255, 223, 186), (255, 255, 153), (255, 140, 0), (25, 25, 112)]

        current_time = time_names[self.simulation.time_of_day]
        current_color = time_colors[self.simulation.time_of_day]

        time_text = self.font.render(f"Time: {current_time} ({self.simulation.step_counter % self.simulation.steps_for_time} / {self.simulation.steps_for_time})", True, current_color)
        screen.blit(time_text, (580, 20))

    def draw_curent_sun_level(self) -> None:
        font = pygame.font.SysFont('Arial', 16)
        sun_text = font.render(f'Sun: {self.simulation.sun_level}', True, self.icon_color)
        screen.blit(sun_text, (580, 70))
    ...
    def draw(self) -> None:
        ...
        self.draw_base_sun_level_buttons()
        self.draw_curent_sun_level()
        self.draw_radio_buttons()
        self.draw_generation()
        self.draw_time_of_day()
...
class EventHandler:
    ...
    def handle_sun_level_buttons(self, mouse_x: int, mouse_y: int) -> None:
        if 300 <= mouse_x <= 340 and 10 <= mouse_y <= 50:
            self.simulation.base_sun_level = max(0, self.simulation.base_sun_level - 1)
        elif 430 <= mouse_x <= 470 and 10 <= mouse_y <= 50:
            self.simulation.base_sun_level = min(16, self.simulation.base_sun_level + 1)

        if self.simulation.time_of_day in [0, 2]: 
            self.simulation.sun_level = max(0, self.simulation.base_sun_level - 1)
        elif self.simulation.time_of_day == 1:  
            self.simulation.sun_level = min(16, self.simulation.base_sun_level + 2)
        else: 
            self.simulation.sun_level = max(0, self.simulation.base_sun_level - 3)
...
class Simulation:
    def __init__(self, started_tree: int = None) -> None:
        ...
        self.base_sun_level = 6
        self.sun_level = self.base_sun_level
        self.ui = UI(self)
        self.cell_grid = {}
        self.occupied_positions = set()
        self.time_of_day = 0
        self.step_counter = 0
        self.steps_for_time = 30
    ...
    def run(self):
        ...
        while self.running:
            ...
            if not self.paused:
                ...
                if self.step_counter % self.steps_for_time == 0 or self.step_counter == 1:
                    if self.step_counter != 1:
                        self.time_of_day = (self.time_of_day + 1) % 4

                    if self.time_of_day in [0, 2]: 
                        self.sun_level = max(0, self.base_sun_level - 1)
                    elif self.time_of_day == 1: 
                        self.sun_level = min(16, self.base_sun_level + 2)
                    else:
                        self.sun_level = max(0, self.base_sun_level - 3)
