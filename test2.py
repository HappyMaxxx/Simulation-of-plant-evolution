...
class Cell:
    ...
    def update_energy(self) -> int:
        ...
        self.update_level()
        self.energy += self.level * max(3 - upper, 0) * self.get_soil_fertility() + 2
    ...
    def get_soil_fertility(self) -> float:
        root = self.tree.cells[0].x
        return self.simulation.soil_blocks[root].fertility
...
class Tree:
    ...
    def birth(self, x, y) -> None:
        if x and y:
            ...
            self.cells.append(Cell(simulation=self.simulation, tree=self, x=random.randint(5, cols-1), y=rows - 2, gen=self.genome.genes[0]))
        ...
        for cell in self.cells:
            if cell.state == '0' and cell.energy >= self.growth_energy:
                ...
                for i, (new_x, new_y) in enumerate(directions):
                    ...
                    if 0 <= new_y < rows - 1 and new_y >= 19 and not (new_x, new_y) in self.simulation.occupied_positions:
                        can_grow = True 
    ...
    def fall_cells(self) -> None:
        for cell in self.cells:
            if cell.y < rows - 2 and not (cell.x, cell.y + 1) in self.simulation.occupied_positions:
                cell.y += 1
            elif cell.y == rows - 2:
                pass
            else:
...
class SoilBlock:
    def __init__(self, simulation: 'Simulation', x: int) -> None:
        self.simulation = simulation
        self.x = x
        self.fertility = random.uniform(0.5, 1)

    def get_color(self):
        base_color = (92, 44, 10)
        light_color = (205, 133, 63)
        
        r = int(base_color[0] * self.fertility + light_color[0] * (1 - self.fertility))
        g = int(base_color[1] * self.fertility + light_color[1] * (1 - self.fertility))
        b = int(base_color[2] * self.fertility + light_color[2] * (1 - self.fertility))
    ...
    def iterate(self):
        self.waste_fertility()
        self.restore_fertility()
...
class TreeDetailsWindow:
    def __init__(self, simulation: 'Simulation', tree: 'Tree') -> None:
        def open_window():
            ...
            tk.Label(left_frame, text=f"Energy: {tree.energy:.2f} ({(tree.getting_energy-tree.waste_energy):.2f})", font=("Arial", 12), bg='#242424', fg='#5E9F61').pack(pady=5)
            tk.Label(left_frame, text=f"Getting: {tree.getting_energy:.2f} Waste: {tree.waste_energy:.2f}", font=("Arial", 12), bg='#242424', fg='#5E9F61').pack(pady=5)
...
class Simulation:
    def __init__(self, started_tree: int = None) -> None:
        ...
        self.trees = []
        self.tree_infos = []
        self.soil_blocks = [SoilBlock(self, x) for x in range(cols)]
        self.roots = {}
            ...
            while not selected_cell:
                for event in pygame.event.get():
                    if event.type == pygame.MOUSEBUTTONDOWN:
                        ...
                        while clicked_cell_y < rows - 2 and not any(c.x == clicked_cell_x and c.y == clicked_cell_y + 1 for tree in self.trees for c in tree.cells):
                            clicked_cell_y += 1
    ...
    def draw_soil(self):
        for i in range(cols):
            color = self.soil_blocks[i].get_color()
            pygame.draw.rect(screen, color, (i * cell_size, (rows - 1) * cell_size, cell_size, cell_size))
        ...
        while self.running:
            ...
            for cell in self.cell_grid.values():
                if self.display_mode == 'normal':
                    ...
                    energy_color = (min(255, max(30, int(cell.last_energy * 5) + 45)), 0, 0)
            ...
            if not self.paused:
                ...
                for soil_block in self.soil_blocks:
                    soil_block.iterate()
